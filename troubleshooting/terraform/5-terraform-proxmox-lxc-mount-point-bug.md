...
## 1. Context
- System: Terraform 1.14.3, bpg/proxmox (initially 0.93.1)
- Environment: Dev (pve-dev)
- Related components: Ansible LXC (CTID 2001), NFS storage (nas-dev-data)

## 2. Issue
- Symptom: When using the `bpg/proxmox` Terraform provider to manage LXC containers with mount_points, the provider fails to create or track mount_points properly, causing perpetual drift and forced container replacement.
- Error:
```
...
Finding: Manual creation works. NFS storage supports mount_points.
...
Finding: Same issue - mount_point not created, state empty.
...
Finding: Non-root users cannot create mount_points. But this wasn't the root cause - the provider wasn't even attempting the API call.
...
Finding: Same issue - Terraform still shows mount_point not in state, triggers replacement.
...
Finding: Would work as workaround but not a real solution.

**Root Cause Discovery - GitHub Issues**
...
## 4. Root Cause
> Provider bug in version 0.93.x. The mount_point schema lacks the "mount point name" (mp0, mp1) attribute, so the provider doesn't know which list item belongs to which mount point. Regression in 0.93.0 forces replacement instead of in-place update. Fixed in v0.94.0+ (PR #2529).

## 5. Solution
> Upgrade provider from 0.93.1 to 0.96.0.
...
## 6. Solution Risk
- Risk level: LOW
- Potential impact: Provider upgrade may introduce other changes - test in dev first

## 7. Impact After Fix
- Observed: Mount point created correctly, state properly tracks mount_point
- No perpetual replacement
...
## 9. Workaround (if any)
> Add `lifecycle { ignore_changes = [mount_point] }` and create mount points manually via `pct set -mp0`.
