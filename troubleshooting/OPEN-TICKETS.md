...
Non-resolved cases across all categories. Last updated: 2026-04-23.

| Ticket | Status | What Happened |
|--------|--------|---------------|
| [TS-IDN-005](identity/5-freeipa-server-sssd-sudo.md) | WORKAROUND APPLIED | Sudo rules work correctly on all IPA clients but not on the FreeIPA server itsel |
| [TS-K8S-010](kubernetes/10-wordpress-admin-password-hash-reset.md) | TEMP CLOSED | WordPress admin login fails with "incorrect password" despite correct password |
...
| [TS-LNX-004](linux/4-cloud-init-etc-hosts-ownership.md) | PENDING (Decision Later) | Ansible playbook k8s_hosts_fallback.yml adds entries to /etc/hosts, but cloud-in |
...
| [TS-PVE-015](proxmox/15-proxmox-crash-during-backup-unknown-cause.md) | UNRESOLVED | REAL INCIDENT -- occurred during unplanned production failure, not planned DR te |
