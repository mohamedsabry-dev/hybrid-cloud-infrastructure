# TS-K8S-009 | 2026-04-04 | IN PROGRESS

## 1. Context

- **System:** Kubernetes Scheduler / Pod Distribution / Resource Management
- **Environment:** k8s-dev and k8s-prod clusters
- **Related Components:** kube-scheduler, anti-affinity, VPA, Descheduler
- **Discovered During:** Investigation of memory over-commitment in Case 8
- **Related:** Case 8 (K8s Dev Cluster Memory Over-Commitment)

---

## 2. Issue

**Symptom:** While investigating memory over-commitment in Case 8, we discovered fundamental limitations in the default Kubernetes scheduler. Pods were not distributing evenly across nodes despite having anti-affinity configured.
...
**Impact:** Uneven pod distribution leading to resource imbalance across nodes.
...
### Step 3: Compare Resource Allocation
...
### Step 4: Verify Anti-Affinity Applied
...
### Step 5: Verify Pod Labels Match
...
### Step 6: Check Scheduling Events
...
### Immediate Fix: Sequential Restart
...
### Long-term Solution: VPA + Descheduler
...
**Chosen Solution:** Vertical Pod Autoscaler (VPA) and Descheduler because:
...
- Anti-affinity configurations for Flux controllers
- Anti-affinity configurations for Ingress controllers

### Prevention Measures

1. Use soft anti-affinity for critical pods (availability > perfect distribution)
2. Right-size requests based on actual usage
3. Implement gradual rebalancing with Descheduler
4. Sequential restarts for proper distribution after anti-affinity changes
...
- **Risk Level:** Low
- **Potential Impact:**
  - Sequential restart: Brief service interruption during restart
  - VPA: May adjust pod resources causing restarts
  - Descheduler: May evict pods for rebalancing
  - Hard anti-affinity can block scheduling entirely (use soft instead)
...
**If pods are unevenly distributed after rolling update:**

**Option A: Sequential restart (recommended)**
```bash
for dep in <deployment-names>; do
...
**Option B: Delete pods one by one**
```bash
...
**Note:** Options B and C may cause brief service interruption. Option A (sequential restart) is preferred as it maintains availability during the process.
