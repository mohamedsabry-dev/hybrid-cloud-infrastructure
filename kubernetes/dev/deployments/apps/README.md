...
│   ├── hpa.yaml                       # 2-3 replicas (CPU 70% / memory 80%)
│   ├── service.yaml                   # ClusterIP + Ingress + SA
│   ├── pvc.yaml                       # 15Gi RWX on nfs-retain
...
|-----|------|-----------|------------|---------|
| **monitoring** | kube-prometheus-stack — Grafana, Prometheus, node-exporter. Scrapes 7 external infra nodes + all K8s workloads | `monitoring` | `grafana` | Grafana 5Gi + Prometheus 20Gi on `nfs-retain` |
...
| **logging** | Loki (single-binary, 50Gi) + Promtail DaemonSet. 7-day retention, filesystem storage | `monitoring` | — | 50Gi on `nfs-retain` |
| **mariadb** | MariaDB 10.11, single instance. InnoDB fsync mode, slow query logging. Backs WordPress | `database` | `mariadb` | 15Gi on `nfs-database` (hard mount) |
| **wordpress** | WordPress 6.9 + Apache. HPA scales 2→3 replicas. Init container waits for MariaDB. Sticky sessions via ingress cookie | `apps` | `wordpress` | 15Gi RWX on `nfs-retain` |
...
| MariaDB | 1 | any node | — |
| WordPress | 2-3 (HPA) | any worker | preferred spread |
