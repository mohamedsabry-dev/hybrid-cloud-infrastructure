...
|--------|-------------|------|
| `golden-template` | Base Rocky Linux LXC source container (used to create the template file) | `lxc, golden, template, prod` |
| `ansible` | Ansible control node for configuration management | `lxc, ansible, automation, prod` |
| `local_runner` | Self-hosted GitHub Actions runner (internal, on-prem) | `lxc, runner, cicd, prod` |
| `nginx` | External reverse proxy / load balancer | `lxc, nginx, proxy, prod` |
| `vault_cluster` | HashiCorp Vault HA cluster (3 nodes, raft) | `lxc, vault, security, prod` |
