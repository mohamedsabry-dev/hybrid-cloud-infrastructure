...
| `golden-template` | Base LXC source container for creating template file | `lxc, golden, template, prod` |
| `local_runner` | CI/CD runner for local pipeline execution | `lxc, runner, cicd, prod` |
| `nginx` | Reverse proxy / load balancer | `lxc, nginx, proxy, prod` |
| `vault_cluster` | HashiCorp Vault HA cluster (3 nodes) | `lxc, vault, security, prod` |
