...
|--------|-------------|------|
| `golden-image` | Rocky Linux 10.1 golden image source VM (install from ISO) | `vm, golden, template, prod` |
| `freeipa` | FreeIPA identity management server | `vm, freeipa, identity, prod` |
| `k8s_masters` | Kubernetes control plane nodes (3 nodes) | `vm, k8s-master, kubernetes, prod` |
| `k8s_workers` | Kubernetes worker nodes (3 nodes) | `vm, k8s-worker, kubernetes, prod` |
