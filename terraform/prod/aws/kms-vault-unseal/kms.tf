  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Key administrators - can manage but not use the key
      {
        ...
        Principal = {
          AWS = [
            "arn:aws:iam::${data.aws_caller_identity.current.account_id}:user/admin_prod",
            "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/GitHubActions-TerraformAdmin-prod"
          ]
        }
        Action = [
          "kms:Create*",
