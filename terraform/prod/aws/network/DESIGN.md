...
(e.g., prod tunnel is `172.17.200.2/16`). That simplifies on-prem routing:
...
- **VPN subnet** (`172.17.55.0/24`): public-facing, hosts the WireGuard
  EC2. Associated with the public route table (internet gateway route).
- **Management subnet** (`172.17.53.0/24`): reserved for internal services
...
naming consistent: `nginx-prod.lab.local` resolves whether you're on-prem
or in the VPC.
