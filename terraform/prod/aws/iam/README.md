...
terraform state mv aws_iam_role.prod_wireguard_ssm aws_iam_role.wireguard_ssm
terraform state mv aws_iam_role_policy_attachment.prod_wireguard_ssm_core aws_iam_role_policy_attachment.wireguard_ssm_core
terraform state mv aws_iam_instance_profile.prod_wireguard_ssm aws_iam_instance_profile.wireguard_ssm
terraform state mv aws_iam_policy.terraform_state_prod aws_iam_policy.terraform_state
terraform state mv aws_iam_policy.security_boundary_prod aws_iam_policy.security_boundary
```

This also triggered AWS resource name changes (forces replacement):
- `prod-wireguard-ssm-role` → `wireguard-ssm-role-prod`
- `prod-wireguard-ssm-profile` → `wireguard-ssm-profile-prod`
- `SecurityBoundary-Prod` → `SecurityBoundary-prod`
- `TerraformState-Prod` → `TerraformState-prod`
